...
AC_DEFUN([OPENAFS_GCC_SUPPORTS_MARCH], [
  AC_CACHE_CHECK([if $CC accepts -march=pentium],
    ...
    [save_CFLAGS="$CFLAGS"
     ...
     AC_COMPILE_IFELSE([AC_LANG_SOURCE([int x;])],[openafs_cv_gcc_supports_march=yes],[openafs_cv_gcc_supports_march=no])
     CFLAGS="$save_CFLAGS"
    ])
...
AC_DEFUN([OPENAFS_GCC_NEEDS_NO_STRICT_ALIASING], [
  AC_CACHE_CHECK([if $CC needs -fno-strict-aliasing],
    ...
    [save_CFLAGS="$CFLAGS"
     ...
     AC_COMPILE_IFELSE([AC_LANG_SOURCE([int x;])],[openafs_cv_gcc_needs_no_strict_aliasing=yes],[openafs_cv_gcc_needs_no_strict_aliasing=no])
     CFLAGS="$save_CFLAGS"
  ])
...
AC_DEFUN([OPENAFS_GCC_NEEDS_NO_STRENGTH_REDUCE], [
  AC_CACHE_CHECK([if $CC needs -fno-strength-reduce],
    ...
    [save_CFLAGS="$CFLAGS"
     ...
     AC_COMPILE_IFELSE([AC_LANG_SOURCE([int x;])],[openafs_cv_gcc_needs_no_strength_reduce=yes],[openafs_cv_gcc_needs_no_strength_reduce=no])
     CFLAGS="$save_CFLAGS"
  ])
...
AC_DEFUN([OPENAFS_GCC_SUPPORTS_NO_COMMON], [
  AC_CACHE_CHECK([if $CC supports -fno-common],
    ...
    [save_CFLAGS="$CFLAGS"
     CFLAGS="-fno-common"
     AC_COMPILE_IFELSE([AC_LANG_SOURCE([int x;])],[openafs_cv_gcc_supports_no_common=yes],[openafs_cv_gcc_supports_no_common=no])
...
AC_DEFUN([OPENAFS_GCC_SUPPORTS_PIPE], [
  AC_CACHE_CHECK([if $CC supports -pipe],
    ...
    [save_CFLAGS="$CFLAGS"
     CFLAGS="-pipe"
     AC_COMPILE_IFELSE([AC_LANG_SOURCE([int x;])],[openafs_cv_gcc_supports_pipe=yes],[openafs_cv_gcc_supports_pipe=no])
  CFLAGS="$save_CFLAGS"
  ])
